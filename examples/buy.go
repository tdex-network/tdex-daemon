...
func BuyExample(daemonAddr, explorerAddr string) error {
	fmt.Printf("---------- BUY LBTC WITH USDt ----------\n\n")
	...
	utxos, err := faucet(explorer, traderWallet, usdt, traderAmount)
	if err != nil {
	...
	swapAccept, err := makeTradeBuyProposal(
